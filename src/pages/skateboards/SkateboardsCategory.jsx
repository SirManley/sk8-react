...
import { useParams } from "react-router-dom"; // ⬅ removed useOutletContext here
...
export default function SkateboardsCategory() {
  ...
  useEffect(() => {
    async function fetchByCategory() {
      ...
      try {
        ...
        // Keep only docs that belong to the "Skateboards" group
        const filtered = docs.filter(
          (item) => Array.isArray(item.groups) && item.groups.includes("Skateboards")
        );
  ...
  if (!items.length)
    return (
      <p className="text-center">No skateboards found for “{displayLabel}.”</p>
    );
