...
export default function SkateboardsHome() {
  const [skateboardItems, setSkateboardItems] = useState([]);
  ...
  useEffect(() => {
    async function fetchItems() {
      ...
      const skateboards = allItems.filter(item =>
        item.groups?.includes('Skateboards')
      );

      const homepageImage =
        skateboards.find(item => item.name === 'Main-Wall') || skateboards[0];

      setFeaturedItem(homepageImage);
      setSkateboardItems(skateboards);
    }
    fetchItems();
  }, []);
  ...
  useEffect(() => {
    let interval = null;
    if (slideshowActive && skateboardItems.length > 1) {
      interval = setInterval(() => {
        setCurrentIndex(prev => (prev + 1) % skateboardItems.length);
      }, 1000);
    }
    return () => clearInterval(interval);
  }, [slideshowActive, skateboardItems]);

  if (!featuredItem) return <p>Loading featured board…</p>;

  const imageToShow = slideshowActive
    ? skateboardItems[currentIndex]
    : featuredItem;
  ...
  const toggleSlideshow = () => {
    if (slideshowActive) {
      ...
      setCurrentIndex(0);
    } else if (skateboardItems.length > 1) {
