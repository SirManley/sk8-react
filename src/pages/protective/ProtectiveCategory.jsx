// src/pages/protective/ProtectiveCategory.jsx
...
export default function ProtectiveCategory() {
  const { category } = useParams();
  const group = "protective";
  ...
  useEffect(() => {
    async function fetchByCategory() {
      ...
      try {
        ...
        // Keep only docs that belong to the "Protective" group
        const filtered = docs.filter(
          (item) => Array.isArray(item.groups) && item.groups.includes("Protective")
        );
  ...
  if (!items.length)
    return (
      <p className="text-center">No protective gear found for “{displayLabel}.”</p>
    );
